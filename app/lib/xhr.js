...
var requestTimeOut = 20000000;
XHR = function() {
};

XHR.prototype.isOnline = function() {
	return Titanium.Network.online;

};
XHR.prototype.getwithOnline = function(url, onSuccess, onError, extraParams) {
	if (!!this.isOnline()) {
	...
	if (!extraParams.ttl || cache == 0) {
		...
		xhr.onload = function() {
			...
			if (extraParams.contentType.indexOf("application/json") != -1) {
				Ti.API.info('GET_DATA: ' + (xhr.responseText));
		...
		xhr.onerror = function(e) {
			...
			onError(result);
			Ti.API.info('errorresult' + JSON.stringify(result));
		};
...
XHR.prototype.getFile = function(url, onSuccess, onError, extraParams, progressFunction) {
	// Debug
	Ti.API.info('******getFile');
	if (url.indexOf('http') == -1)
		url = Alloy.Globals.root + url;
	...
	if (!extraParams.ttl || cache == 0) {
		...
		xhr.ondatastream = function(e) {
			Ti.API.info('ondatastream ' + e.progress);
			if (progressFunction && typeof progressFunction === "function") {
				progressFunction(e.progress);
			}
		};
		xhr.onsendstream = function(e) {
			Ti.API.info('onsendstream ' + e.progress);
			if (progressFunction && typeof progressFunction === "function") {
				progressFunction(e.progress);
			}
		};
		Ti.API.info('******getFile url ' + url);
		xhr.open("GET", url, extraParams.async);
...
// @extraParams (object)
XHR.prototype.post = function(url, data, onSuccess, onError, extraParams, progressFunction) {
	...
	if (!Ti.Network.online) {
		...
		xhr.onload = function(e) {
			...
			result.status = xhr.status == 200 ? "ok" : xhr.status;
			Ti.API.info('POST_DATA: ' + xhr.responseText);
		...
		xhr.onerror = function(e) {
			...
			result.data = e.error;
			Ti.API.info('result.data>>>>>>>' + JSON.stringify(result.data));
			//alert(result.data);
			result.code = xhr.status;
			onError(result);
		};
