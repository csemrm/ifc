var width = Ti.Platform.displayCaps.platformWidth, height = Ti.Platform.displayCaps.platformHeight;
...
function resize(image) {
	var pH = Ti.Platform.displayCaps.platformHeight, pW = Ti.Platform.displayCaps.platformWidth;
	var wR = pW / image.w, hR = pH / image.h, r = Math.max(wR, hR);
	return {
...
function createImageView(image) {
	...
	view1.add(Ti.UI.createImageView({
		...
		left : -1 * (size.width - width) / 2,
		top : -1 * (size.height - height) / 2,
	}));
...
exports.slideView = function(images, time, direction, pagingControlBottom) {
	...
	var viewS1 = [], viewS2 = [], viewS3 = [], currentSV = null, nextSV = null, tempSV = null, prevSV = null, //
	mainView = Ti.UI.createView({
		zIndex : -1,
		width : width,
		height : height,
		backgroundColor : '#fff'
	}), scView1 = creteScrollableView(-1 * width, 'scView1'), scView2 = creteScrollableView(0, 'scView2'), scView3 = creteScrollableView(width, 'scView3');
