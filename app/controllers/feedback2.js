...
function feedback2WinFocus() {
    ...
    if (!!Alloy.Globals.feedbackFormData['data[Feedback][problem]']) {
        ...
        if ($.MLTextArea.value.toString().length == 0) {
    ...
    if (!!Alloy.Globals.feedbackFormData['data[Feedback][message]']) {
        ...
        if ($.solTextArea.value.toString().length == 0) {
            $.nextBtn.enabled = false;
            $.nextBtn.backgroundColor = "#67cee3";
            $.nextBtn.color = "#3dbfd9";
        }
...
if (Alloy.Globals.OS == 'android') {
    var recordedVideo = Titanium.Filesystem.getFile(Ti.Filesystem.externalStorageDirectory, 'feedback.mp4');
} else {
...
function recordVideoBtnClick() {
    Ti.API.info('====recordVideoBtnClick====');
    clearTimer();
        ...
        success : function(e) {
            if (e.success) {
                ...
                if (recordedVideo.exists()) {
                    ...
                    if (Alloy.Globals.OS == 'android') {
                        recordedVideo = Titanium.Filesystem.getFile(Ti.Filesystem.externalStorageDirectory, 'feedback.mp4');
                    } else {
                ...
                if (Alloy.Globals.OS == 'android') {
                    $.videoPlayer.url = Ti.Filesystem.externalStorageDirectory + '/' + 'feedback.mp4';
                } else {
                    $.videoPlayer.media = e.media;
                }
